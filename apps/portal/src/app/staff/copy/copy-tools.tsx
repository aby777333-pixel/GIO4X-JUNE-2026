...
import { setProviderStatus, type PendingProvider } from "@/lib/copy-actions";
import type { Enums } from "@gio4x/supabase";

export function ProviderStatusButtons({ provider }: { provider: PendingProvider }) {
  const router = useRouter();
  const [pending, startTransition] = useTransition();
  const [err, setErr] = useState<string | null>(null);

  function run(status: Enums<"copy_provider_status">) {
    setErr(null);
    startTransition(async () => {
      const res = await setProviderStatus(provider.id, status);
